...
-- En la base de datos es necesario hacer una busqueda exhaustiva acerca de las transacciones, 
-- por esto se realizo una consulta trayendo la informacion detallada de una compra 
    SELECT 
...
-- Es necesario tener toda la informacion acerca de las devoluciones para llevar un control sobre si mismas
-- con esto tenemos una consulta acerca de la informacion necesaria y especifica para gestionar una devolucion de
-- manera correcta 
...
-- Al momento de realizar una venta es necesario tener en cuenta 
-- el empleado que la realizo, todo con el fin de llevar un 
-- orden, en este caso tenemos una consulta en donde nos muestra informacion
-- del cliente y asimismo del empleado
-- para llevar un orden exhaustivo
   SELECT
    t.id AS transaccion_id,
...
-- Al momento de realizar una devolucion es necesario tener informacion clara
-- sobre la compra para asi poder llevar a cabo
-- un tramite exitoso, por lo mismo tenemos una consulta que nos trae informacion 
-- acerca de la transaccion en este caso
-- la compra y de la devolucion
...
-- obtiene un listado de transacciones junto con la información del cliente
-- (nombre, apellido, y tipo de documento) para aquellos clientes que tienen un tipo de documento 'DNI'. 
SELECT
    t.id AS transaccion_id,
    t.fecha_transaccion AS fecha,
    t.monto_total AS monto,
...
-- La idea es identificar y mostrar al empleado que generaron montos mayores al promedio 
-- resalta a los empleados que están manejando transacciones de mayor valor en comparación con el promedio general
SELECT
    e.id AS empleado_id,
    e.nombre AS empleado_nombre,
    e.puesto AS empleado_puesto,
...
-- Evalua el desempeño de los empleados en términos de ventas efectivas, excluyendo las transacciones canceladas que 
-- no contribuyen al total de ventas
...
-- muestra un listado de empleados junto con el monto total de las transacciones que gestionaron, si es que realizaron alguna--
-- Esto asegura que todos los empleados sean incluidos, los resultados incluyen sus respectivos identificadores 
-- muestra un listado de empleados junto con el monto total de las transacciones que gestionaron, si es que realizaron alguna.
    ...
    -- La consulta muestra el nombre y apellido de los clientes que han realizado transacciones
    -- La cláusula WHERE filtra los resultados para incluir solo
    -- aquellos clientes cuyas transacciones superen el monto especificado.
    SELECT
    c.nombre AS cliente_nombre,
    c.apellido AS cliente_apellido,
    t.monto_total AS monto
...
-- muestra los productos con la descripcion que suministra el proveedor--
-- Esto asegura que se incluyan todos los proveedores, mostrando los tipos de uniformes que suministran con el fin de llevar a cabo un orden
-- dentro de la categoria o tipo que se encuentre
	SELECT p.id AS proveedor_id, p.nombre AS proveedor_nombre, u.tipo AS uniforme_tipo
	FROM proveedores p
	LEFT JOIN uniformes u ON p.id = u.id;

--  muestra todos los clientes que realizaron una transacion, utiliza un LEFT JOIN para asegurarse de que se incluyan todos los clientes
-- esto permite identificar a los clientes que han realizado alguna, con el valor del monto correspondiente
...
-- Consulta para obtener las transacciones realizadas por clientes cuyo nombre empiece con 'A', 
-- y que hayan pagado con el mismo método de pago más de una vez, se realiza con el fin de hacer una busqueda especifica 
-- de algun cliente 
...
-- Consulta que encuentra el cliente que ha realizado la mayor cantidad de transacciones
-- que tuvieron el estado "pendientes" y 
-- devuelve los detalles de su última transacción, con el fin de 
-- llevar un orden y descargas dependiendo del estado que se encuentre
...
--  Consulta para especificar detalles como devoluciones de transacciones completadas, incluyendo información sobre el cliente, 
-- el empleado responsable y los detalles del pago, en este caso con el fin de realizar una busqueda precisa sobre 
-- algun cliente en especifico 
...
-- Consulta para obtener el detalle de las transacciones y devoluciones,
--  con su estado, realizadas por
-- cada cliente durante un periodo específico, 
-- esto para poder buscar informacion necesaria y urgente teniendo 
-- datos como la fecha
